...
<body>
    ...
    <section class="features-section">
        ...
        <div class="features-grid">
            ...
            <div class="feature-card" style="display: flex; flex-direction: column;">
                <h3>AI Brain Tumor Detector</h3>
                <p style="flex-grow: 1;">
                    Upload an MRI or brain scan image, and our advanced deep learning model will analyze it to detect
                    potential brain tumors. The system classifies the scan and provides an instant prediction with
                    high accuracy using state-of-the-art AI techniques.
                </p>
                <div class="ai-links" style="margin-bottom: 10px; margin-top: auto;">
                    <button class="ai-btn open-modal submit-btn" data-target="#modalBrain">Upload & Detect</button>
                </div>
            </div>
...
<div id="modalBrain" class="health-modal">
    <div class="health-modal-content">
        <span class="health-modal-close" onclick="document.getElementById('modalBrain').style.display='none'">&times;</span>
        <h2>AI Brain Tumor Detection</h2>
        
        <p class="disclaimer" style="color: #a00; font-weight: bold; margin-bottom: 20px; padding: 10px; border: 1px solid #f00; border-radius: 4px; background-color: #ffeaea;">
            Disclaimer: This is an AI support tool and not a medical diagnosis.
        </p>

        <form class="health-form" id="brain-tumor-form">
            <div class="form-group" style="margin-bottom: 15px;">
                <label>Upload Brain MRI Scan</label>
                <input type="file" name="brain_image" class="form-control" accept="image/*" required>
            </div>

            <div class="form-group text-center">
                <img id="previewImage" src="#" alt="Preview" style="display:none; max-width:100%; height: 200px; border: 1px solid #ccc; margin-bottom: 15px;">
            </div>

            <button type="submit" class="health-submit">Analyze Image</button>
        </form>

        <div id="result" style="margin-top: 20px; color:#000" ></div>
    </div>
</div>

<script>
    const imageInput = document.querySelector('input[name="brain_image"]');
    const previewImage = document.getElementById('previewImage');
    const form = document.getElementById('brain-tumor-form');
    ...
    form.addEventListener('submit', async function (e) {
        ...
        resultDiv.innerHTML = "<em>Processing image... Please wait.</em>";

        const formData = new FormData();
        formData.append('file', imageInput.files[0]); // Must match PHP key

        try {
            const response = await fetch('brain_predict.php', {
                method: 'POST',
                body: formData
            });
            ...
            if (data.error) {
                ...
                // UI Color Coding
                const color = data.prediction === 'No Tumor' ? 'green' : 'red';
                
                resultDiv.innerHTML = `
                    <div style="border: 2px solid ${color}; padding: 15px; border-radius: 8px;">
                        <h3 style="color:${color}; margin-top:0;">${data.prediction}</h3>
                        <p><strong>Confidence:</strong> ${data.confidence}</p>
                        <small>Detection breakdown:</small>
                        <ul style="font-size: 0.85em; list-style: none; padding: 0;">
                            ${Object.entries(data.all_probs).map(([k, v]) => `<li>${k}: ${(v*100).toFixed(2)}%</li>`).join('')}
                        </ul>
                    </div>
                `;
            }
        } catch (err) {
            resultDiv.innerHTML = `<div style="color:red">Network Error. Check PHP server.</div>`;
        }
    });
</script>
...
<script>
    document.getElementById("downloadPDF").addEventListener("click", async () => {
        ...
        pdf.text("Generated by WellTrackeR | © 2026", startX, footerY2);
